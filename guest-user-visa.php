...
$status=2;
$cancelby='a';
$sql = "UPDATE tblvisa SET status=:status,CancelBy=:cancelby WHERE  VisaId=:bid";
...
$status=1;
$cancelby='a';
$sql = "UPDATE tblvisa SET status=:status WHERE VisaId=:bcid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
<?php if($result->status==2)
{
	?><td>Cancelled</td>
<?php } else {?>
<td><a href="guest-user-visa.php?bkid=<?php echo htmlentities($result->VisaId);?>" onclick="return confirm('Do you really want to cancel booking')" >Cancel</a> / <a href="guest-user-visa.php?bckid=<?php echo htmlentities($result->VisaId);?>" onclick="return confirm('Do you really want to confirm booking')" >Confirm</a></td>
<?php }?>
