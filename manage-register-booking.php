...
$sql = "UPDATE tbl_flight_hotel_booking SET status=:status,CancelledBy=:cancelby,updated_at=:pdate WHERE BookingId=:bid";
...
$cancelby='a';
$pdate = date('Y-m-d H:i:s');
$sql = "UPDATE tbl_flight_hotel_booking SET status=:status,updated_at=:pdate WHERE BookingId=:bcid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_INT);
$query -> bindParam(':pdate',$pdate , PDO::PARAM_STR);
				...
				<div class="agile-tables">
					<div class="w3l-table-info">
					  <h2>Manage Application Process</h2>
						...
						<tbody>
<?php $sql = "SELECT tbl_flight_hotel_booking.BookingId as BookId, tbl_flight_hotel_booking.TravelCategory as Category,
tbl_flight_hotel_booking.TravelPerson as TotalPerson, tbl_flight_hotel_booking.DepartureDate as Ddate , 
...
tbl_flight_hotel_booking.CheckIn as CheckIn, tbl_flight_hotel_booking.CheckOut as CheckOut,tbl_flight_hotel_booking.updated_at AS updated_at,
...
{				?>		
						  <tr>
							<td>#FHB-<?php echo htmlentities($result->BookId);?></td>
							<td><?php echo htmlentities($result->FullName);?></td>
							<td><?php echo htmlentities($result->MobileNo);?></td>
                            
                            <td><?php echo htmlentities($result->FlightName);?></td>
							<td><?php echo htmlentities($result->Departurecity);?></td>
                            <td><?php echo htmlentities($result->Ddate);?></td>
...
{
echo '<span class="badge badge-primary">Canceled by you at </span><br/>' .$result->updated_at;
} 
...
<?php } else {?>
<td><a href="#" id="<?php echo htmlentities($result->BookId);?>" class="view_data" data-toggle="modal" data-target="#exampleModal">View</a>&nbsp; / &nbsp;<a href="manage-register-booking.php?bkid=<?php echo htmlentities($result->BookId);?>" onclick="return confirm('Do you really want to cancel booking')" style="color: red;">Cancel</a> &nbsp; / &nbsp;<a href="manage-register-booking.php?bckid=<?php echo htmlentities($result->BookId);?>" onclick="return confirm('Do you really want to confirm booking')" style="color: #4DB321;">Confirm</a></td>
<?php }?>
